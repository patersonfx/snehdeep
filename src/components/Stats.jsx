...
export default function Stats() {
  return (
    <section className="bg-primary py-12 sm:py-16 md:py-20 px-4 sm:px-6 md:px-10 lg:px-16 relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {stats.map((s, i) => (
            <div key={i} className="text-center group">
              <div className="inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-white/10 text-accent mb-3 sm:mb-4 group-hover:bg-accent group-hover:text-white transition-all duration-300">
                {s.icon}
              </div>
              <p className="font-heading text-3xl sm:text-4xl md:text-5xl font-bold text-accent mb-1">{s.value}</p>
